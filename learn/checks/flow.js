...
(async () => {
  ...
  assert(REDUCED || await page.evaluate(() => { const g = [...document.querySelectorAll("body > .bubble")]; return g.length >= 1 && g.every(x => x.style.visibility !== "hidden"); }), "continue: result and incoming ghosts fly visibly");
  await settle(1000);
  v = await view();
  ...
  assert(await page.evaluate(() => document.getElementById("sheet-progress").dataset.state) === "peek", "peek stays while no keyboard is up: " + await page.evaluate(() => document.getElementById("sheet-progress").dataset.state));
  assert(await page.evaluate(() => { const b = document.querySelector("#answer-form .primary").getBoundingClientRect(), s = document.getElementById("sheet-progress").getBoundingClientRect(); return b.bottom <= s.top || b.top >= s.bottom; }), "check button is not under the peek strip");
  await page.evaluate(() => window.Learn.sheet.suspend()); await settle(700);
  assert(await page.evaluate(() => document.getElementById("sheet-progress").hidden === true), "suspend hides the peek for the keyboard");
  await page.evaluate(() => window.Learn.sheet.resume()); await settle(700);
  assert(await page.evaluate(() => document.getElementById("sheet-progress").dataset.state) === "peek", "resume brings the peek back");
  ...
  assert(await page.evaluate(() => { const r = document.getElementById("toast").getBoundingClientRect(); return !document.getElementById("toast").hidden && Math.abs((r.left + r.right) / 2 - innerWidth / 2) < 4; }), "toast is centred");
  ...
  assert(await page.evaluate(() => document.getElementById("tabbar").inert === true), "tab bar is inert under the open sheet");
  assert(await page.evaluate(() => document.querySelector("#sheet-pill .pill-thumb").offsetWidth > 60), "sheet pill thumb is placed when the sheet opens");
  await page.click("#sheet-progress label.row:has(#opt-sound)", {position: {x: 30, y: 20}}); await settle(300);
  assert(await page.evaluate(() => document.getElementById("opt-sound").checked === true), "tapping a row label toggles its option");
